...
export default function LaporDiri() {
  ...
  // FAKTA: Fungsi Validasi NIK Tingkat Lanjut (Matematika Tanggal & Anti-Spam)
  const validasiNIKLogika = (nikInput: string, pemilik: string): string | null => {
    ...
    // Tolak angka berulang (1111111111111111) atau berurutan (1234567890123456)
    if (/^(\d)\1{15}$/.test(nikInput)) return `NIK ${pemilik} terdeteksi spam (angka berulang).`;
    if (nikInput === "1234567890123456" || nikInput === "0123456789012345") return `NIK ${pemilik} tidak valid.`;

    // Ekstrak Tanggal Lahir (Digit 7-12) -> DDMMYY
    const tglLahirStr = nikInput.substring(6, 12);
    let hari = parseInt(tglLahirStr.substring(0, 2));
    const bulan = parseInt(tglLahirStr.substring(2, 4));
    const tahunStr = parseInt(tglLahirStr.substring(4, 6));

    // Koreksi untuk wanita (ditambah 40)
    if (hari > 40) hari -= 40;

    // Validasi range kalender dasar
    if (hari === 0 || hari > 31) return `Digit hari pada NIK ${pemilik} tidak rasional.`;
    if (bulan === 0 || bulan > 12) return `Digit bulan pada NIK ${pemilik} tidak rasional.`;

    // Logika abad (Asumsi kasar: jika tahun < tahun sekarang (26), berarti 2000-an, jika tidak 1900-an)
    const currentYearStr = parseInt(new Date().getFullYear().toString().substring(2, 4));
    const fullYear = tahunStr > currentYearStr ? 1900 + tahunStr : 2000 + tahunStr;
    
    // Verifikasi validitas tanggal nyata (misal: bukan 30 Februari)
    const dateObj = new Date(fullYear, bulan - 1, hari);
    if (dateObj.getFullYear() !== fullYear || dateObj.getMonth() !== bulan - 1 || dateObj.getDate() !== hari) {
       return `Kombinasi tanggal lahir pada NIK ${pemilik} tidak sesuai kalender masehi.`;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 1. Eksekusi Benteng Validasi PIN & NIK Kepala Keluarga
    const pinLemah = ["123456", "111111", "000000", "654321", "121212", "123123"];
    if (pinLemah.includes(pin)) return alert("PIN terlalu gampang ditebak! Gunakan kombinasi 6 angka acak.");
    
    const errorNikKK = validasiNIKLogika(nik, "Kepala Keluarga");
    if (errorNikKK) return alert(errorNikKK);

    // 2. Eksekusi Benteng Validasi NIK Anggota Keluarga
    for (let i = 0; i < anggota.length; i++) {
      const errorNikAnggota = validasiNIKLogika(anggota[i].nik, anggota[i].nama || `Anggota ke-${i+1}`);
      if (errorNikAnggota) return alert(errorNikAnggota);
    }

    if (!dokumenMenyusul) {
      if (!fileKtp) return alert("Lampirkan Foto KTP, atau centang opsi 'Dokumen Menyusul' di bawah jika belum ada.");
      if (!fileKk) return alert("Lampirkan Foto Kartu Keluarga, atau centang opsi 'Dokumen Menyusul' di bawah jika belum ada.");
    }
    ...
    try {
      ...
      const { data: dataWarga, error: errorWarga } = await supabase
        .from("warga")
        .insert([{ nik, nama_lengkap: nama, no_whatsapp: wa, pin, status_tinggal: statusTinggal, detail_alamat: detailAlamat, ktp_path: pathKtp, kk_path: pathKk }])
        .select();
      ...
      if (anggota.length > 0 && dataWarga) {
        ...
        const payloadAnggota = anggota.map((a) => ({ warga_id: dataWarga[0].id, nama_lengkap: a.nama, nik: a.nik, hubungan_keluarga: a.hubungan }));
      ...
      <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-4 pb-4 border-b border-slate-200 bg-slate-100 p-4 rounded-xl border border-slate-300">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
              ...
              <label className="flex items-center gap-2 cursor-pointer bg-white px-3 py-1.5 rounded-lg border border-amber-300 text-amber-900 font-bold text-xs shadow-sm hover:bg-amber-50 select-none">
                <input type="checkbox" checked={dokumenMenyusul} onChange={(e) => { setDokumenMenyusul(e.target.checked); if (e.target.checked) { setFileKtp(null); setFileKk(null); } }} className="w-4 h-4 text-amber-600 rounded" />
                Dokumen Menyusul
              </label>
            </div>
            ...
            {!dokumenMenyusul ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm">
                  <label className="block text-xs font-bold text-slate-700 mb-2">📸 Foto E-KTP (KK)</label>
                ...
                <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm">
                  <label className="block text-xs font-bold text-slate-700 mb-2">📸 Foto Kartu Keluarga</label>
              ...
              <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-amber-800 text-xs font-medium">⚠️ Wajib menyerahkan salinan KTP & KK fisik ke rumah Pak RT agar akun disahkan.</div>
            )}
          </div>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center bg-slate-100 p-2 rounded">
              <div>
                ...
                <p className="text-xs text-slate-500">Sistem otomatis validasi tanggal lahir pada NIK.</p>
              </div>
              ...
              <div key={index} className="flex flex-col md:flex-row gap-3 items-start md:items-center bg-slate-50 p-4 rounded-lg border border-slate-200">
                <div className="flex-1 w-full space-y-3">
                  ...
                  <div className="flex gap-2">
                    <input type="text" maxLength={16} minLength={16} required className="w-2/3 border border-slate-300 rounded p-2 text-sm outline-none focus:border-blue-500 text-slate-900 font-mono" placeholder="NIK (16 Digit)" value={item.nik} onChange={(e) => ubahAnggota(index, "nik", e.target.value.replace(/[^0-9]/g, ''))} />
          ...
          <button type="submit" disabled={loading} className={`w-full text-white text-lg font-bold rounded-xl p-4 transition-all mt-6 shadow-md relative overflow-hidden ${loading ? 'bg-slate-700 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 hover:shadow-lg'}`}>
            {loading ? <span className="flex items-center justify-center gap-2"><span className="animate-spin">⌛</span> {progressTeks}</span> : "Kirim Data Lapor Diri"}
          </button>
        </form>
      </div>
    </div>
