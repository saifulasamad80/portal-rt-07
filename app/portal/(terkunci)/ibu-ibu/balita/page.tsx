...
export default async function PosyanduBalitaPage() {
  ...
  const { data } = await supabase.from("posyandu_balita").select("*").order("tanggal_kunjungan", { ascending: false }).limit(50);

  return (
    <KerangkaIbuIbu
      judul="Posyandu Balita"
      deskripsi="Pantauan tumbuh kembang, berat badan, tinggi, dan imunisasi balita RT."
    >
