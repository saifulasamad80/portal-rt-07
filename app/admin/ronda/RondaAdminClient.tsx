...
export default function RondaAdminClient({ adminAktif, jadwalList, wargaList, aksiSimpan, aksiHapus }: { adminAktif: any, jadwalList: any[], wargaList: any[], aksiSimpan: any, aksiHapus: any }) {
  ...
  const handleSimpan = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setWargaId(""); 
      // Sengaja tanggal ga di-reset biar gampang kalau mau input regu di hari yang sama
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 md:p-10 font-sans">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <Link href="/admin" className="text-slate-600 font-bold text-sm hover:underline mb-2 inline-block">
        ...
        <div className="bg-slate-900 p-6 md:p-8 rounded-2xl shadow-lg border-l-[12px] border-amber-500 mb-8 flex justify-between items-center">
          <div>
            <h1 className="text-2xl md:text-3xl font-black text-white mb-1">Manajemen Siskamling</h1>
            <p className="text-slate-400 text-sm">Atur regu ronda malam dan pantau status kehadiran warga.</p>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 lg:col-span-1 h-fit border-t-[6px] border-t-amber-500">
            ...
            <form onSubmit={handleSimpan} className="space-y-4">
              <div>
                ...
                <input type="date" required className="w-full border-2 border-slate-200 rounded-lg p-3 outline-none focus:border-amber-500 text-sm font-bold text-slate-700" value={tanggal} onChange={(e) => setTanggal(e.target.value)} />
              </div>
              
              <div>
                <label className="block text-xs font-bold text-slate-600 mb-2 uppercase tracking-wide">Pilih Petugas (Warga)</label>
                <select required className="w-full border-2 border-slate-200 rounded-lg p-3 outline-none focus:border-amber-500 text-sm font-bold text-slate-700" value={wargaId} onChange={(e) => setWargaId(e.target.value)}>
            ...
            <div className="overflow-x-auto max-h-[600px] overflow-y-auto rounded-xl border border-slate-200">
              <table className="w-full text-left border-collapse text-sm">
                <thead className="sticky top-0 z-10 bg-slate-100 text-slate-700 text-xs">
                  <tr>
                    <th className="p-4 border-b-2 border-slate-200">Tanggal Tugas</th>
                    <th className="p-4 border-b-2 border-slate-200">Nama Petugas</th>
                    <th className="p-4 border-b-2 border-slate-200">Status & Keterangan</th>
                    <th className="p-4 border-b-2 border-slate-200 text-center">Aksi</th>
                  </tr>
                </thead>
                <tbody>
                  {jadwalList.length === 0 ? (
                    ...
                    jadwalList.map((j) => (
                      <tr key={j.id} className="border-b border-slate-200 hover:bg-slate-50 transition-colors">
                        <td className="p-4">
                          <div className="font-black text-slate-800 bg-slate-200 px-2 py-1 rounded inline-block">
                            {new Date(j.tanggal_tugas).toLocaleDateString('id-ID', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' })}
                          </div>
                        </td>
                        <td className="p-4 font-black text-slate-700">{j.warga?.nama_lengkap}</td>
                        <td className="p-4">
                          <span className={`text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded shadow-sm ${
                            j.status === 'Menunggu Konfirmasi' ? 'bg-slate-200 text-slate-600' :
                          ...
                          {j.alasan_izin && (
                            <div className="text-[10px] text-slate-500 italic mt-2 border-l-2 border-rose-300 pl-2">
                              Alasan: {j.alasan_izin}
                            </div>
                          )}
                        </td>
                        <td className="p-4 text-center">
                          <button onClick={() => handleHapus(j.id, j.warga?.nama_lengkap)} disabled={deleteLoadingId === j.id} className="text-[10px] font-bold text-rose-500 hover:bg-rose-50 border border-rose-200 px-3 py-1.5 rounded transition-colors disabled:opacity-50">
                            Batal
                          </button>
                        </td>
                      </tr>
