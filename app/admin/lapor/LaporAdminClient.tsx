...
export default function LaporAdminClient({ adminAktif, laporanList, aksiTanggapi }: { adminAktif: any, laporanList: any[], aksiTanggapi: any }) {
  ...
  // REVISI UX: Deteksi otomatis apakah data diubah atau masih sama dengan aslinya
  const isUnchanged = laporanAktif && status === (laporanAktif.status || "Menunggu") && tanggapan === (laporanAktif.tanggapan_rt || "");
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pt-2">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 lg:col-span-2 h-fit">
            ...
            <div className="space-y-4 max-h-[700px] overflow-y-auto pr-2 custom-scrollbar">
              {laporanList.length === 0 ? (
                ...
                laporanList.map((l) => (
                  // REVISI UX: Hapus border merah (error association). Ganti ke warna seleksi biru pudar.
                  <div key={l.id} onClick={() => bukaPanelTanggapan(l)} className={`p-5 border rounded-xl cursor-pointer transition-all shadow-sm ${laporanAktif?.id === l.id ? 'border-blue-400 bg-blue-50 shadow-md' : 'border-slate-200 bg-white hover:border-blue-200 hover:bg-slate-50'}`}>
                    ...
                    <div className="flex justify-between items-start gap-4 mb-3">
                      ...
                      {/* REVISI UX: Sistem Pill Badge (Hijau, Kuning, Biru) */}
                      <span className={`text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full shrink-0 ${
                        (!l.status || l.status === 'Menunggu') ? 'bg-amber-100 text-amber-800' :
                        l.status === 'Diproses' ? 'bg-blue-100 text-blue-800' :
                        l.status === 'Selesai' ? 'bg-emerald-100 text-emerald-800' :
          ...
          <div className="lg:col-span-1">
            {laporanAktif ? (
              // REVISI UX: Hapus border merah tebal
              <div className="bg-white p-6 rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.05)] border border-slate-200 sticky top-6">
                ...
                <form onSubmit={handleSimpan} className="space-y-4">
                  <div>
                    ...
                    <select className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-blue-500 text-sm font-bold text-slate-700 bg-white" value={status} onChange={(e) => setStatus(e.target.value)}>
                  ...
                  {/* REVISI UX: Jarak margin bottom ditambahkan */}
                  <div className="mb-4">
                    <label className="block text-[10px] font-black text-slate-500 mb-2 uppercase tracking-widest">Tanggapan Resmi Pengurus</label>
                    <textarea required rows={5} className="w-full border border-slate-300 rounded-lg p-3 outline-none focus:border-blue-500 text-sm text-slate-800" placeholder="Ketik jawaban atau tindak lanjut dari laporan ini..." value={tanggapan} onChange={(e) => setTanggapan(e.target.value)} />
                  </div>
                  
                  {/* REVISI UX: Logic disable state aktif jika tidak ada yang diubah */}
                  <button type="submit" disabled={loadingId === laporanAktif.id || isUnchanged} className={`w-full text-white font-bold rounded-lg p-3.5 mt-2 transition-all ${loadingId === laporanAktif.id || isUnchanged ? 'bg-slate-300 text-slate-500 cursor-not-allowed shadow-none' : 'bg-blue-600 hover:bg-blue-700 shadow-md active:scale-95'}`}>
