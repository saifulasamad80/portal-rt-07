...
export default async function AdminIbuIbuPage() {
  ...
  const [lansiaRes, balitaRes, arisanRes, transaksiRes] = await Promise.all([
    supabase.from("kunjungan_lansia").select("*").order("tanggal_kunjungan", { ascending: false }).limit(200),
    supabase.from("kunjungan_balita").select("*").order("tanggal_kunjungan", { ascending: false }).limit(200),
